...
<body class="bg-gray-50 min-h-screen">
            ...
            <div class="p-6 lg:p-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 mb-6">
                    ...
                    <div class="stat-card bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-800 mt-1">{{ $totalResponden }}</p>
                    ...
                    <div class="stat-card bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-medium text-gray-500">Rata-rata Kepuasan</p>
                                <div class="text-2xl font-bold text-gray-800 mt-1 flex items-baseline">
                                    <span>{{ $satisfactionAverage }}</span>
                                    <span class="text-sm font-normal ml-1 text-gray-600">%</span>
                                </div>
                                <p class="text-xs text-gray-400 mt-1">Dari seluruh survey berskala</p>
                            </div>
                    ...
                    <div class="stat-card bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-800 mt-1">{{ $totalAdmin }}</p>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                    <!-- Chart for Responden Stats - Takes 2 columns -->
                    <div class="lg:col-span-2">
                        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 h-full">
                            <div class="flex justify-between items-center mb-6">
                                <h3 class="text-base font-semibold text-gray-800">Statistik Responden</h3>
                                <div class="flex space-x-2">
                                    <button id="dailyToggle" class="px-3 py-1.5 rounded-lg text-sm font-medium bg-primary-100 text-primary-700 border border-primary-200">Harian</button>
                                    <button id="monthlyToggle" class="px-3 py-1.5 rounded-lg text-sm font-medium text-gray-600 bg-gray-100 border border-gray-200">Bulanan</button>
                                </div>
                            </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <table class="w-full">
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    ...
                                    <tr class="hover:bg-gray-50/50 transition-colors">
                                        <td class="px-5 py-4">
                                            <p class="text-sm font-medium text-gray-800 truncate max-w-[160px]" title="{{ $form->nama }}">{{ $form->nama }}</p>
                                            <p class="text-xs text-gray-500">{{ $form->respondens->count() }} responden</p>
                                        </td>
                                        <td class="px-5 py-4">
                    ...
                    <!-- Upcoming Expired Forms -->
                    <div class="bg-white rounded-xl border border-gray-100 shadow-sm">
                        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                            <div>
                                <h3 class="text-base font-semibold text-gray-800">Survey Mendekati Akhir</h3>
                                <p class="text-xs text-gray-400 mt-0.5">Form survey yang akan segera berakhir</p>
                            </div>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full">
                                <thead>
                                    <tr class="bg-gray-50/50">
                                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama Form</th>
                                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tanggal Berakhir</th>
                                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Status</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-100">
                                    @if($upcomingExpiredForms->count() > 0)
                                        @foreach($upcomingExpiredForms as $form)
                                        <tr class="hover:bg-gray-50/50 transition-colors">
                                            <td class="px-5 py-4">
                                                <p class="text-sm font-medium text-gray-800 truncate max-w-[200px]" title="{{ $form->nama }}">{{ $form->nama }}</p>
                                            </td>
                                            <td class="px-5 py-4">
                                                <p class="text-sm text-gray-600">{{ \Carbon\Carbon::parse($form->tanggal_selesai)->format('d M Y') }}</p>
                                            </td>
                                            <td class="px-5 py-4">
                                        ...
                                        <tr>
                                            <td colspan="3" class="px-5 py-12 text-center">
                                                <div class="flex flex-col items-center">
                                                    ...
                                                    <p class="text-sm text-gray-500">Tidak ada survey yang akan berakhir dalam waktu dekat</p>
                                                </div>
                                            </td>
                                        </tr>
                                    @endif
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const dailyData = @json($respondenStats['daily']);
            const monthlyData = @json($respondenStats['monthly']);
            ...
            const ctx = document.getElementById('respondenChart').getContext('2d');
            let currentChart = new Chart(ctx, {
                type: 'line',
                data: {
                    labels: dailyLabels,
                    datasets: [{
                        label: 'Jumlah Responden Harian',
                        data: dailyCounts,
                        borderColor: '#3B82F6',
            ...
            document.getElementById('dailyToggle').addEventListener('click', function() {
                document.getElementById('dailyToggle').classList.add('bg-primary-100', 'text-primary-700');
                document.getElementById('dailyToggle').classList.remove('bg-gray-100', 'text-gray-600');
                document.getElementById('monthlyToggle').classList.add('bg-gray-100', 'text-gray-600');
                document.getElementById('monthlyToggle').classList.remove('bg-primary-100', 'text-primary-700');
            ...
            document.getElementById('monthlyToggle').addEventListener('click', function() {
                document.getElementById('monthlyToggle').classList.add('bg-primary-100', 'text-primary-700');
                document.getElementById('monthlyToggle').classList.remove('bg-gray-100', 'text-gray-600');
                document.getElementById('dailyToggle').classList.add('bg-gray-100', 'text-gray-600');
                document.getElementById('dailyToggle').classList.remove('bg-primary-100', 'text-primary-700');
                ...
                currentChart = new Chart(ctx, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            label: 'Jumlah Responden Bulanan',
                            data: monthlyCounts,
                            borderColor: '#10B981',
